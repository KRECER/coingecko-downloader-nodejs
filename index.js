...
const createTokenIcon = async (address, iconUrl, chainName) => {
  const folderToBeCreatedPath = path.resolve(process.cwd(), 'tokens', chainName);
...
const downloadCoingeckoIcons = async (chainId) => {
  ...
  const response = await axios.get(`${hostname}/en${chainNameQueryParam}`);
  ...
  for (let i = 1; i <= pagesCountTotal; i++) {
    ...
    const pageCurrentResponse = await axios.get(`${hostname}/en${chainNameQueryParam}${currentPageQueryParam}`);

    $ = cheerio.load(pageCurrentResponse.data);
    const tokenElems = $(selectors.tokenTableRow);

    tokenElems.each(async (_, elem) => {
      const tokenLogoAbsoluteLink = $(elem).find(selectors.tokenTableRowLogo).attr('data-src');
      const tokenDetailsRelativeLink = $(elem).find(selectors.tokenTableRowLink).attr('href');
      ...
      const currentTokenAddress = $(selectors.tokenDetailsAddress)['0'].attribs['data-address'];

      if (currentTokenAddress.length > 30) {
        console.log('currentTokenAddress', currentTokenAddress);
        await createTokenIcon(currentTokenAddress, tokenLogoAbsoluteLink, chainName);
      }
